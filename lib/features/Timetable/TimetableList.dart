...
class _TimetableListState extends State<TimetableList> {
  ...
  Future<void> _loadTimetables() async {
    ...
    try {
      ...
      final timetables = snapshot.docs
          .map((doc) => TimetableInfo.fromFirestore(doc))
          .toList();
  ...
  Future<void> _addTimetableToFirestore(TimetableInfo newTimetable) async {
    ...
    await docRef.set({
      ...
      'color': newTimetable.color.value.toRadixString(16), // Color 객체를 16진수 문자열로 변환
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('시간표 목록', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        actions: [
          Padding(
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          Text("${timetable.year}년", style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                          Text(timetable.semester, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  );
...
class _AddTimetableModalState extends State<AddTimetableModal> {
  ...
  void _addTimetable() {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final tableName = "${year}년 $semester";
      // 입력된 정보로 새로운 TimetableInfo 객체를 생성합니다.
      final newTimetable = TimetableInfo(
        tableName: tableName, year: year, semester: semester,
        color: _selectedColor, createdAt: Timestamp.now(),
      );
        ...
        child: Padding(
            ...
            child: Form(
                ...
                child: Column(
                    ...
                    children: [
                      const Text('새 시간표 추가', textAlign: TextAlign.center, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          ElevatedButton(onPressed: _addTimetable, child: const Text('추가')),
                        ],
                      ),
                    ]))));
  }
